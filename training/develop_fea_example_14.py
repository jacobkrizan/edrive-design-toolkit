"""develop_fea_example_14.py - Motor FEA with Torque Waveform Analysis via Rotor Rotation
========================================================================================

This script builds a complete FEA model of a surface-mounted PM motor with 
selectable linear or nonlinear steel material properties, user-controlled mesh sizing,
independent control of PM magnets and 3-phase distributed windings, and parametric
rotor rotation for torque waveform analysis with synchronized current control.

Key Features:
- **Rotor Rotation Study**: Parametric sweep of rotor mechanical angle
  * Compute torque at multiple rotor positions (cogging + electromagnetic)
  * Synchronized current: advances electrically with rotor to maintain constant torque angle
  * Rotor geometry and magnet Hc vectors rotate together
  * Generate complete torque waveform showing ripple characteristics
  * FFT analysis for harmonic content identification (6th, 12th harmonics for 48-slot, 8-pole)
  * ENABLE_ROTATION_STUDY flag to switch between single-position and rotation modes
- **Synchronized Current Physics**: Correct implementation for constant torque angle
  * WINDING_CURRENT_ANGLE: Base torque angle (0° for d-axis, 90° for q-axis)
  * Current phase advances electrically with rotor rotation
  * Formula: current_angle = WINDING_CURRENT_ANGLE + rotor_angle_elec
  * Maintains fixed MMF-to-rotor relationship throughout rotation
  * Torque ripple from slot effects and winding harmonics, not fundamental variation
- **Independent Excitation Control**: User variables to control:
  * MAGNET_HC: Permanent magnet coercive field in A/m (set to 0 to disable)
  * WINDING_CURRENT_DENSITY: Three-phase current density in A/m² (set to 0 to disable)
  * WINDING_CURRENT_ANGLE: Base torque angle in electrical degrees
- **Three-Phase Distributed Winding**: 48 slots, 8 poles
  * Proper 120° electrical phase shift between phases
  * Pattern repeats every 12 slots (one pole pair)
  * Creates 8-pole rotating MMF (4 north, 4 south)
  * Fixed spatial distribution in stator frame
  * Temporal currents rotate via phase angle control
...
- **User-Controlled Mesh Sizing**: Independent mesh size controls:
  * Rotor core, Magnets, Airgap (critical), Stator core, Windings, Air regions
  * Airgap mesh influences stator tooth tips and slot openings for accuracy
- **Optimized Rotor Geometry**: 0.1mm gap between rotor core and magnets (surface-mounted)
- **User-Controlled Air Regions**: Total air thickness beyond stator OD, auto-split at midpoint
...
- Five air regions: center air, inner air, middle air 1, middle air 2, shell
...
Rotation Study Physics:
- Rotor mechanical angle: 0° to 45° (one electrical cycle for 8-pole motor)
- Rotor electrical angle: θ_elec = θ_mech × (poles/2) = θ_mech × 4
- Magnet Hc vectors rotate with geometry to maintain physical alignment
- Current angle synchronized: θ_current = θ_base + θ_elec
- Result: Constant torque angle, ripple from slot harmonics only

Winding Pattern (48 slots, 8 poles):
- Each pole pair spans 12 slots
- Pattern: A+ A+ C- C- B+ B+ A- A- C+ C+ B- B-
- Phase A: slots 0-1, 6-7, 12-13, 18-19, 24-25, 30-31, 36-37, 42-43
- Phase B: slots 4-5, 10-11, 16-17, 22-23, 28-29, 34-35, 40-41, 46-47
- Phase C: slots 2-3, 8-9, 14-15, 20-21, 26-27, 32-33, 38-39, 44-45
- Polarity alternates to create 4 north poles and 4 south poles

User Configuration (top of file):
- ENABLE_ROTATION_STUDY: True to run multi-position torque analysis, False for single position
- ROTATION_STEPS: Number of rotor positions to analyze (default 6-24)
- ROTATION_START_DEG / ROTATION_END_DEG: Mechanical angle range (default 0° to 45°)
- MAGNET_HC: Permanent magnet coercive field (A/m, set to 0 for cogging torque only)
- WINDING_CURRENT_DENSITY: Three-phase current density (A/m², set to 0 to disable)
- WINDING_CURRENT_ANGLE: Base torque angle in degrees (0° for d-axis, 90° for q-axis)
- USE_NONLINEAR_STEEL: True/False for analysis mode
- MESH_ROTOR_CORE: Maximum element size in rotor core (default 3mm)
- MESH_MAGNET: Maximum element size in magnets (default 2mm)
- MESH_AIRGAP: Maximum element size at airgap and tooth tips (default 0.5mm, critical)
- MESH_STATOR_CORE: Maximum element size in stator core (default 3mm)
- MESH_WINDING: Maximum element size in windings (default 2mm)
- MESH_AIR_GLOBAL: Maximum element size in air regions (default 10mm)
- TOTAL_AIR_THICKNESS: Total air space beyond stator OD (default 10mm)
...
- **Electromagnetic Torque**: Maxwell stress tensor integration along airgap
- **Torque Waveform**: Parametric rotation study with FFT harmonic analysis
- **Flux Linkage**: Airgap flux integration method with winding distribution factor
- **Motor Constants**: Torque constant (Kt), back-EMF constant (Ke), performance metrics
- Comprehensive torque and flux linkage visualization plots
- All output files and plots saved to develop_fea_example_14_output/

Expected Results:
- Torque waveform with small ripple around constant average (~46-47 N-m for example motor)
- Dominant harmonics at 6th and 12th orders (slot harmonics for 48-slot, 8-pole)
- Torque ripple typically 1-5% for well-designed distributed winding motors
- No low-order harmonics (1st, 2nd) when synchronized current is correctly implemented
...
work_dir = os.path.join(script_dir, 'develop_fea_example_14_output')
...
# Mesh size controls (in mm)
MESH_ROTOR_CORE = 3.0      # Maximum element size in rotor core
MESH_MAGNET = 2.0          # Maximum element size in magnets
MESH_AIRGAP = 0.5          # Maximum element size at airgap (critical for accuracy)
MESH_STATOR_CORE = 3.0     # Maximum element size in stator core
MESH_WINDING = 2.0         # Maximum element size in windings
MESH_AIR_GLOBAL = 10.0     # Maximum element size in air regions

# Winding geometry controls
WINDING_CLEARANCE = 0.5    # Clearance between winding and slot walls (mm), for insulation

# Excitation controls (set to zero to disable)
MAGNET_HC = 900000.0           # A/m - magnet coercive field (set to 0 to disable PM excitation)
WINDING_CURRENT_DENSITY = 5e6  # A/m² - winding current density (set to 0 to disable winding excitation)
WINDING_CURRENT_ANGLE = 0.0      # degrees - electrical angle for torque/position analysis

# Rotation study controls (for torque waveform analysis)
ENABLE_ROTATION_STUDY = False   # Set to True to compute torque at multiple rotor positions
ROTATION_STEPS = 6             # Number of rotor positions to analyze
ROTATION_START_DEG = 0.0       # Starting mechanical angle (degrees)
ROTATION_END_DEG = 45.0        # Ending mechanical angle (degrees, 45° = 1 elec cycle for 8-pole)

# Air region geometry (in mm beyond stator OD)
TOTAL_AIR_THICKNESS = 10.0  # Total air space outside stator OD
                            # Boundary splits this into two equal regions
...
// Generated by develop_fea_example_14.py
...
def create_solver_file(magnet_hc, winding_current_density, winding_current_angle, use_nonlinear_steel, rotor_angle_deg=0.0):
    ...
    mm_to_m = 0.001
    num_slots = params.num_slots  # Get number of slots for winding definitions
...
"""
    ...
    winding_phases = assign_winding_phases(num_slots, params.num_poles)
...
"""
    ...
    print(f"    - Magnets: μr≈{params.magnet_mu_r}")
    print(f"  Regions:")
...
def calculate_torque_maxwell(b_pos_file, params):
    """
    ...
    airgap = params.airgap_length * mm_to_m
    airgap_middle_r = rotor_outer_r + airgap / 2
    stack_length = params.stack_length * mm_to_m
    ...
    rotor_volume = np.pi * (rotor_outer_r**2 - (params.rotor_inner_diameter/2*mm_to_m)**2) * stack_length
...
def calculate_flux_linkage(b_pos_file, params, phase='A'):
    """
    ...
    winding_phases = assign_winding_phases(params.num_slots, params.num_poles)
    ...
    turns_per_coil = params.turns_per_coil
...
def calculate_all_phases_linkage(b_pos_file, params):
    ...
    for phase in ['A', 'B', 'C']:
        result = calculate_flux_linkage(b_pos_file, params, phase)
...
def display_motor_constants(torque_result, linkage_results, params):
    """
    ...
    mm_to_m = 0.001
    slot_depth = params.slot_depth * mm_to_m
    slot_width = params.slot_width * mm_to_m
    ...
    # Calculate specific power if total mass is available
    if hasattr(params, 'total_motor_mass') and params.total_motor_mass > 0:
        print(f"{'Specific Power:':<30} {power_base_kW/params.total_motor_mass:>10.2f} kW/kg")
...
def plot_torque_and_linkage(torque_result, linkage_results, params):
    """
    ...
    for phase in ['A', 'B', 'C']:
        phase_angles = []
        phase_polarities = []
        for slot_idx in range(params.num_slots):
            slot_phase, polarity = winding_phases[slot_idx]
            if slot_phase == phase:
                angle = slot_idx * 2 * np.pi / params.num_slots
    ...
    mm_to_m = 0.001
    slot_area = params.slot_depth * params.slot_width * mm_to_m * mm_to_m
    ...
    Poles / Slots:       {params.num_poles} / {params.num_slots}
    Stack Length:        {params.stack_length:.1f} mm
    Airgap Length:       {params.airgap_length:.2f} mm
    """
    ...
    # Overall title
    fig.suptitle(f'{params.num_poles}-Pole, {params.num_slots}-Slot Motor - Electromagnetic Performance Analysis',
...
def run_rotation_study(params):
    ...
    Args:
        params: MotorParameters object with motor geometry
    ...
    num_pole_pairs = params.num_poles / 2
    ...
    for i, angle in enumerate(angles_mech):
        ...
        if os.path.exists(b_pos_file):
            torque_result = calculate_torque_maxwell(b_pos_file, params)
...
def plot_torque_waveform(rotation_results, params):
    """
    ...
    Analysis Configuration:
      ...
      Number of poles:           {params.num_poles} ({params.num_poles/2:.0f} pole pairs)
    ...
    Torque Statistics:
      ...
      Torque density:            {torque_mean / (np.pi * (params.rotor_outer_diameter/2000)**2 * params.stack_length/1000):.2f} kN-m/m³
    ...
    fig.suptitle(f'{params.num_poles}-Pole Motor: {analysis_type} Waveform Analysis',
...
def main():
    ...
    if ENABLE_ROTATION_STUDY:
        ...
        # Run rotation study
        rotation_results = run_rotation_study(params)
        
        # Plot results
        plot_torque_waveform(rotation_results, params)
    ...
    if os.path.exists(b_pos_file) and (MAGNET_HC > 0 or WINDING_CURRENT_DENSITY > 0):
        linkage_results = calculate_all_phases_linkage(b_pos_file, params)
    else:
    ...
    if torque_result and linkage_results:
        motor_constants = display_motor_constants(torque_result, linkage_results, params)
        
        # Step 10: Create torque and flux linkage visualization
        plot_torque_and_linkage(torque_result, linkage_results, params)
    else:
