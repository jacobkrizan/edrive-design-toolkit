"""develop_fea_example_13.py - Motor FEA with Independent PM and 3-Phase Winding Control
====================================================================================
...
and independent control of PM magnets and 3-phase distributed windings.
...
- **User-Controlled Mesh Sizing**: Five independent mesh size controls at top of file:
  * Rotor core, Magnets, Airgap (critical), Stator core, Air regions
...
- Five air regions: center air, inner air, middle air 1 (airgap to slots), 
  middle air 2 (stator OD to midpoint), and shell (midpoint to boundary)
...
Geometry Structure:
- Center air: 0 to rotor core ID (shaft hole)
- Rotor core: steel annulus (shaft ID to rotor core OD, 0.1mm gap to magnets)
...
- Middle air 2: stator OD to (stator OD + TOTAL_AIR_THICKNESS/2)
- Shell: (stator OD + TOTAL_AIR_THICKNESS/2) to (stator OD + TOTAL_AIR_THICKNESS)

Slot Implementation:
- Each slot defined by 4 points (pt1, pt2 at inner radius; pt3, pt4 at slot depth)
- Bottom points (pt1, pt2) use airgap mesh size for refined tooth tips
- Top points (pt3, pt4) use stator mesh size
...
- MAGNET_HC: Permanent magnet coercive field (A/m, set to 0 to disable)
- WINDING_CURRENT_DENSITY: Three-phase current density (A/m², set to 0 to disable)
- WINDING_CURRENT_ANGLE: Electrical angle for torque analysis (degrees)
...
- All output files and plots saved to develop_fea_example_13_output/

Expected Results:
- Complete motor FEA with physically accurate slotted stator geometry
- Air mesh fills slot cavities
- Independent control of PM and winding excitation sources
- Proper 8-pole rotating MMF from distributed 3-phase windings
...
- Refined mesh at tooth tips captures accurate field gradients at airgap interface
- Torque calculation from Maxwell stress tensor (industry-standard method)
- Flux linkage from airgap flux integration with winding factor
- Accurate motor constants for performance prediction
"""
...
work_dir = os.path.join(script_dir, 'develop_fea_example_13_output')
...
// Generated by develop_fea_example_13.py
...
def create_solver_file(magnet_hc, winding_current_density, winding_current_angle, use_nonlinear_steel):
    """
...
def main():
    ...
    pro_file = create_solver_file(MAGNET_HC, WINDING_CURRENT_DENSITY, WINDING_CURRENT_ANGLE, USE_NONLINEAR_STEEL)
    if not pro_file:
