"""develop_fea_example_7.py - Motor FEA with Stator Core Donut and Robust Air Regions
====================================================================================

Final version: This script builds a robust FEA model of a surface-mounted PM motor 
with a steel rotor core and a stator core donut (no slots yet), using a clean, 
multi-region air structure for reliable meshing and analysis.

Key Features:
- Steel rotor core (μr=2000) with shaft hole
- Stator core as a solid donut (no slots)
- Five air regions: center air, inner air, middle air 1 (airgap middle to stator ID), 
  middle air 2 (stator OD to outer air), and shell
- Dynamic parameter loading from Excel (projects/example_design_1/)
- 8-pole surface-mounted PM rotor, 48-slot stator (geometry ready for slotting)
- Fine mesh at airgap for accurate field calculation
- Python-based post-processing and visualization (matplotlib)
...
- Middle air 1: airgap middle to stator ID (pre-stator air)
- Stator core: stator ID to stator OD (solid donut)
- Middle air 2: stator OD to outer air (post-stator air)
- Shell: outer air to domain boundary (far-field)

Post-Processing:
- Complete motor cross-section plot (shaft, rotor, magnets, stator, air regions)
- 2D flux density colormap and airgap flux profile
- All output files and plots saved to develop_fea_example_7_output/

Expected Results:
- Robust mesh and solution for all regions
- Ready for slotting and further FEA analysis
"""
...
work_dir = os.path.join(script_dir, 'develop_fea_example_7_output')
...
def create_geometry():
    ...
    geo_content = f"""// {params.num_poles} Arc Magnets with Rotor Core and Stator Teeth - Motor Parameters
// Generated by develop_fea_example_7.py
...
Circle(300) = {{300, 1, 301}};  // Stator inner arc (quadrant 1)
Circle(301) = {{301, 1, 302}};  // Stator inner arc (quadrant 2)
Circle(302) = {{302, 1, 303}};  // Stator inner arc (quadrant 3)
Circle(303) = {{303, 1, 300}};  // Stator inner arc (quadrant 4)
...
Curve Loop(321) = {{300, 301, 302, 303}};  // Stator inner boundary (hole)
Plane Surface(322) = {{320, 321}};  // Stator core - solid donut

// Middle air region 1 (airgap middle to stator inner - air gap before stator)
Curve Loop(225) = {{300, 301, 302, 303}};  // Stator inner boundary (outer)
Curve Loop(226) = {{190, 191, 192, 193}};  // Airgap middle boundary (inner)
Plane Surface(227) = {{225, 226}};  // Air annulus before stator
...
"""
    ...
    print(f"  Stator core: R={stator_inner_r/mm_to_m:.1f}-{stator_outer_r/mm_to_m:.1f}mm (steel donut, no slots yet)")
...
Function {{
  ...
  // Stator core: Steel with relative permeability μr=2000 (linear approximation)
