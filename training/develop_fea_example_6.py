"""develop_fea_example_6.py - Complete Motor FEA with Steel Rotor Core
==============================================================================

This example adds a realistic steel rotor core to the motor FEA model,
providing accurate flux paths through magnetic material.

Features:
- Steel rotor core (μr=2000) with shaft hole
- Separate center air region (0 to rotor ID)
- Four air regions for complete field solution
- Dynamic parameter loading from Excel file (projects/example_design_1/)
- Complete motor geometry visualization (shaft, rotor core, magnets, stator, slots)
- 8-pole surface-mounted PM motor with 48 slots
- Fine mesh at airgap for accurate field calculation
- Python-based post-processing with matplotlib

Motor Configuration (from Excel):
- Rotor: 50mm ID (shaft), 138mm OD
- Rotor core: 25mm ID, 60mm OD (steel, μr=2000)
- Magnets: 4mm thick, 80% arc ratio, surface-mounted at 65-69mm
- 5mm gap between rotor core OD and magnet ID
- Airgap: 1mm (between magnet OD and stator ID)
- Stator: 148mm ID (calculated), 200mm OD
- Slots: 4mm width, 12mm depth, 1mm opening
- Electrical: 100Vdc battery, 100A rated current

Geometry Structure:
- Rotor core: 25-60mm radius (steel annulus with shaft hole)
- Center air: 0-25mm radius (separate region)
- Inner air: 25-69.5mm radius with rotor core and magnet cutouts
- Outer air: 69.5-102mm radius (airgap middle to stator OD + 2mm)
- Shell: 102-110mm radius (far-field boundary)
- Alternating N-S polarity with radial magnetization

Post-Processing:
- Complete motor cross-section plot with all components
- 2D flux density colormap across entire geometry
- Airgap flux density profile vs angular position
- High-resolution PNG output for documentation

Expected Results:
- Flux density: 0.0-0.76 T peak
- Airgap flux: sinusoidal 8-pole pattern
- Mesh: ~12,000 nodes, ~24,000 elements
- Solver: ~11,600 DOFs, convergence < 1e-10
"""
...
work_dir = os.path.join(script_dir, 'develop_fea_example_6_output')
...
def create_geometry():
    """
    Step 1: Create the geometry file (.geo) with 3-region air structure
    
    Builds complete motor geometry with dynamic parameters from Excel:
    - 8 arc magnets on rotor surface (N-S alternating polarity)
    - Inner air region: center to airgap middle with magnet cutouts
    - Outer air region: airgap middle to stator OD + 2mm
    - Shell region: stator OD + 2mm to stator OD + 10mm (far-field BC)
    - Fine mesh at airgap boundary for accurate field calculation
    ...
    geo_content = f"""// {params.num_poles} Arc Magnets with Rotor Core - Motor Parameters
// Generated by develop_fea_example_6.py
...
Curve Loop(226) = {{190, 191, 192, 193}};  // Airgap middle boundary
Plane Surface(227) = {{225, 226}};  // Outer air annulus
...
Physical Surface("Outer Air", 111) = {{227}};
...
"""
    ...
    print(f"  Inner air: R=0-{airgap_middle_r/mm_to_m:.1f}mm (with rotor core and magnet cutouts)")
    print(f"  Outer air: R={airgap_middle_r/mm_to_m:.1f}-{outer_air_r/mm_to_m:.1f}mm")
...
def create_solver_file():
    ...
    pro_content = f"""// {params.num_poles} Arc Magnets with Steel Rotor Core - Motor Parameters
// Generated by develop_fea_example_6.py
...
"""
  ...
  Vol_Mag = Region[ {{RotorCore, CenterAir, InnerAir, OuterAir, AirInf, {all_magnets_list}}} ];
...
Function {{
  ...
  // Rotor core: Steel with relative permeability μr=2000 (linear approximation)
  mu_r_steel = 2000.;
  nu [ Region[ RotorCore ] ] = 1. / (mu_r_steel * mu0);
  mu [ Region[ RotorCore ] ] = mu_r_steel * mu0;
  
  // Air regions and magnets: μr=1
  nu [ Region[{{CenterAir, InnerAir, OuterAir, AirInf, {all_magnets_list}}} ] ] = 1. / mu0;
  mu [ Region[{{CenterAir, InnerAir, OuterAir, AirInf, {all_magnets_list}}} ] ] = mu0;
...
"""
    ...
    print(f"  Materials:")
    print(f"    - Rotor core: Steel (μr=2000)")
