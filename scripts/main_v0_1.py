...
class MotorDesignApp:
    ...
    def create_control_panel(self, parent):
        ...
        # View Mesh in Gmsh button
        gmsh_btn = ttk.Button(scrollable_frame, text="View Mesh in Gmsh",
                             command=self.open_gmsh_viewer)
        gmsh_btn.pack(pady=5, padx=10, fill=tk.X)
        self.button_widgets.append(gmsh_btn)
    ...
    def update_all_plots(self):
        ...
        # Tab 3: Airgap Flux (MEC)
        fig3 = self.plot_figures["Airgap Flux (MEC)"]
        fig3.clear()
        ax3 = fig3.add_subplot(111)
        if mec_solution:
            self.visualizer.plot_airgap_flux(ax3, mec_solution)
        self.plot_canvases["Airgap Flux (MEC)"].draw()
        
        # Tab 4: Airgap Flux (FEA)
        fig4 = self.plot_figures["Airgap Flux (FEA)"]
        fig4.clear()
        ax4 = fig4.add_subplot(111)
        if self.fea_solution is not None:
            self.visualizer.plot_fea_airgap_flux(ax4, self.fea_solver, self.fea_solution)
        else:
            ax4.text(0.5, 0.5, 'No FEA Results\n\nSelect "FEA" or "Both" solver mode',
                    ha='center', va='center', fontsize=14, color='gray')
            ax4.axis('off')
        self.plot_canvases["Airgap Flux (FEA)"].draw()
        
        # Tab 5: MEC Diagram
        fig5 = self.plot_figures["MEC Diagram"]
        fig5.clear()
        ax5 = fig5.add_subplot(111)
        if mec_solution:
            self.visualizer.plot_mec_diagram(ax5, mec_solution)
        self.plot_canvases["MEC Diagram"].draw()
        
        # Tab 6: FEA Mesh with Geometry Overlay
        fig6 = self.plot_figures["FEA Mesh"]
        fig6.clear()
        if self.fea_solution is not None and 'mesh' in self.fea_solution:
            ax6 = fig6.add_subplot(111)
            self.visualizer.plot_fea_mesh(ax6, self.fea_solution['mesh'], self.rotor_angle)
        else:
            ax6 = fig6.add_subplot(111)
            ax6.text(0.5, 0.5, 'No FEA Mesh\n\nSelect "FEA" or "Both" solver mode',
                    ha='center', va='center', fontsize=14, color='gray')
            ax6.axis('off')
        self.plot_canvases["FEA Mesh"].draw()
        
        # Tab 7: FEA Results (if available)
        fig7 = self.plot_figures["FEA Results"]
        fig7.clear()
        if self.fea_solution is not None:
            ax7 = fig7.add_subplot(111)
            self.visualizer.plot_fea_results(ax7, self.fea_solution)
        else:
            ax7 = fig7.add_subplot(111)
            ax7.text(0.5, 0.5, 'No FEA Results\n\nSelect "FEA" or "Both" solver mode',
                    ha='center', va='center', fontsize=14, color='gray')
            ax7.axis('off')
...
"""
    ...
    def open_gmsh_viewer(self):
        """Open the FEA mesh in Gmsh viewer"""
        import subprocess
        import os
        import tempfile
        from tkinter import messagebox
        
        # Check if FEA solution exists with mesh
        if self.fea_solution is None or 'mesh' not in self.fea_solution:
            messagebox.showwarning("No Mesh Available",
                                 "Please run FEA analysis first.\n\nSelect 'FEA' or 'Both' solver mode and click 'Update Design'.")
            return
        ...
        gmsh_paths = [
            ...
            os.path.join(os.path.dirname(__file__), "..", "src", "gmsh", "gmsh.exe")
        ]
        
        gmsh_exe = None
        for path in gmsh_paths:
            if os.path.exists(path):
                gmsh_exe = path
                break
        
        if gmsh_exe is None:
            messagebox.showerror("Gmsh Not Found",
                               "Gmsh executable not found.\n\nPlease install Gmsh from: https://gmsh.info")
            return
        
        # Get mesh file path from FEA solver
        mesh_file = None
        if hasattr(self.fea_solver, '_last_mesh_file') and self.fea_solver._last_mesh_file:
            mesh_file = self.fea_solver._last_mesh_file
        ...
        # Open Gmsh with the mesh file
        try:
            subprocess.Popen([gmsh_exe, mesh_file])
            messagebox.showinfo("Gmsh Viewer",
                              f"Opening mesh in Gmsh...\n\nMesh file: {os.path.basename(mesh_file)}")
        except Exception as e:
            messagebox.showerror("Error", f"Could not open Gmsh:\n{str(e)}")
