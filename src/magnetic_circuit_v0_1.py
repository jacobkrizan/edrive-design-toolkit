...
class MagneticCircuitSolver:
    ...
    def solve_mec(self, rotor_angle: float = 0.0) -> Dict:
        ...
        # Ground the first rotor node - this is more symmetric than grounding yoke
        # since rotor is the return path and is shared by all poles equally
        ground_idx = self.rotor_nodes[0]
        G[ground_idx, :] = 0
    ...
    def _calculate_fluxes_from_potentials(self, potentials: np.ndarray, reluctances: Dict, rotor_angle: float) -> Dict:
        ...
        Returns individual tooth fluxes for proper torque and voltage calculations
        in three-phase motors. Each tooth flux is calculated separately to enable
        phase flux linkage computation.
        """
        fluxes = {
            'airgap': [],
            'teeth': [],  # Individual flux through each tooth
            'tooth_flux_linkage': {},  # Flux linkage per phase
            'yoke': [],
            'magnets': [],
            'rotor': []
        }
        ...
        # Individual tooth fluxes - critical for torque/voltage calculation
        winding_config = self.geometry.get_winding_positions()
        
        # Initialize phase flux linkage tracking
        phase_flux_linkage = {'A': 0.0, 'B': 0.0, 'C': 0.0}
        
        for slot_idx in range(n_slots):
            tooth_node = self.tooth_nodes[slot_idx]
            yoke_node = self.yoke_nodes[slot_idx]
            
            # Calculate flux through this individual tooth
            tooth_flux = (potentials[yoke_node] - potentials[tooth_node]) / reluctances['tooth']
            fluxes['teeth'].append(tooth_flux)
            
            # Accumulate flux linkage for each phase
            # Find which phase and polarity this slot belongs to
            for phase, windings in winding_config.items():
                for winding in windings:
                    if winding['slot_index'] == slot_idx:
                        # Flux linkage = flux * turns * polarity
                        flux_linkage_contribution = tooth_flux * winding['turns'] * winding['polarity']
                        phase_flux_linkage[phase] += flux_linkage_contribution
                        break
        
        # Store phase flux linkages
        fluxes['tooth_flux_linkage'] = phase_flux_linkage
    ...
    def calculate_performance(self, rotor_angle: float = 0.0) -> PerformanceMetrics:
        """Calculate all performance metrics using individual tooth fluxes"""
        ...
        airgap_flux_densities = [abs(f) / airgap_area for f in fluxes['airgap']]
        metrics.airgap_flux_density_peak = max(airgap_flux_densities)
        
        # Individual tooth flux densities
        tooth_area = self.params.tooth_width * self.params.stack_length * 1e-6
        tooth_flux_densities = [abs(f) / tooth_area for f in fluxes['teeth']]
        ...
        # Back EMF calculation using phase flux linkages from individual teeth
        winding_config = self.geometry.get_winding_positions()
        
        # Use the flux linkage calculated from individual tooth fluxes
        phase_flux_linkages = fluxes['tooth_flux_linkage']
        
        # Average flux linkage across phases (should be similar in balanced motor)
        avg_flux_linkage = np.mean([abs(phase_flux_linkages[p]) for p in ['A', 'B', 'C']])
        metrics.flux_linkage = avg_flux_linkage / self.params.parallel_paths
        
        # Back EMF: e = ω * λ
        omega = 2 * np.pi * self.params.rated_speed / 60.0
        metrics.back_emf = omega * metrics.flux_linkage / np.sqrt(2)  # RMS value
        
        # Torque calculation using simplified approach: T = (3/2) * (P/2) * λ * I
        # This is the average torque for a balanced three-phase motor
        metrics.torque = (3.0 / 2.0) * (self.params.num_poles / 2.0) * metrics.flux_linkage * self.params.rated_current
        
        # Torque ripple estimation (sweep rotor angle to find variation)
        torque_angles = np.linspace(0, 2 * np.pi / self.params.num_poles, 18)
        torques = []
        
        for angle in torque_angles:
            sol = self.solve_mec(angle)
            phase_fl = sol['fluxes']['tooth_flux_linkage']
            avg_fl = np.mean([abs(phase_fl[p]) for p in ['A', 'B', 'C']]) / self.params.parallel_paths
            T = (3.0 / 2.0) * (self.params.num_poles / 2.0) * avg_fl * self.params.rated_current
            torques.append(T)
        
        metrics.torque_ripple = (np.max(torques) - np.min(torques)) / np.mean(torques) * 100 if np.mean(torques) != 0 else 0
        ...
        # Calculate turns per phase from winding configuration
        winding_config = self.geometry.get_winding_positions()
        turns_per_phase = sum([w['turns'] for w in winding_config.get('A', [])])
